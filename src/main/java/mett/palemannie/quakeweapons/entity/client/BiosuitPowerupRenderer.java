...
public class BiosuitPowerupRenderer extends EntityRenderer<BiosuitPowerupEntity, EntityRenderState> {
    ...
    private static final Identifier BIOSUIT_LOCATION = Identifier.fromNamespaceAndPath(QuakeWeapons.MODID,"textures/entity/biosuit_powerup/biosuit_powerup.png");
    ...
    public void submit(EntityRenderState state, PoseStack poseStack, SubmitNodeCollector nodeCollector, CameraRenderState cameraState) {
        ...
        nodeCollector.submitModel(this.model, state, poseStack, this.model.renderType(BIOSUIT_LOCATION), state.lightCoords, OverlayTexture.NO_OVERLAY, state.outlineColor, null);

        nodeCollector.submitModel(this.model, state, poseStack, RenderTypes.eyes(BIOSUIT_LOCATION), state.lightCoords, OverlayTexture.NO_OVERLAY, state.outlineColor, null);
