...
import mett.palemannie.quakeweapons.entity.custom.ShellsAmmopickupEntity;
...
public class ShellsAmmopickupRenderer extends EntityRenderer<ShellsAmmopickupEntity, EntityRenderState> {
    ...
    private static final Identifier SHELLS_AMMOPICKUP_LOCATION = Identifier.fromNamespaceAndPath(QuakeWeapons.MODID,"textures/entity/ammopickups/shells_ammopickup.png");
    private final ShellsAmmopickupModel model;

    public ShellsAmmopickupRenderer(EntityRendererProvider.Context context) {
        super(context);
        this.model = new ShellsAmmopickupModel(context.bakeLayer(ShellsAmmopickupModel.SHELLS_AMMOPICKUP_LAYER));
    }
    ...
    public void submit(EntityRenderState state, PoseStack poseStack, SubmitNodeCollector nodeCollector, CameraRenderState cameraState) {
        ...
        nodeCollector.submitModel(this.model, state, poseStack, this.model.renderType(SHELLS_AMMOPICKUP_LOCATION), state.lightCoords, OverlayTexture.NO_OVERLAY, state.outlineColor, null);
        nodeCollector.submitModel(this.model, state, poseStack, RenderTypes.eyes(SHELLS_AMMOPICKUP_LOCATION), state.lightCoords, OverlayTexture.NO_OVERLAY, state.outlineColor, null);
